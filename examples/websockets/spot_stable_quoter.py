...
from derive_client.data.generated.models import Direction, OrderStatus, PublicGetTickerResultSchema
from derive_client.data_types import Environment
from derive_client.data_types.enums import OrderSide, OrderType

BASES = ["USDT", "DAI", "USDe", "PYUSD"]
QUOTES = ["USDC"]
MAX_POSTION_SIZE = 100
QUOTE_SIZE = 0.1
BUY_OFFSET = 0.99
SELL_OFFSET = 1.01
...
class WebsocketQuoterStrategy:
    ...
    def on_ticker_update(self, orderbook: Orderbook):
        breakpoint()
    ...
    def create_order(self, market: str, side: OrderSide, price: float, amount: float) -> OrderResponseSchema:
        ...
        for side in [Direction.buy, Direction.sell]:
            ...
            for ix, (nonce, order) in enumerate(current_orders.copy().items()):
                if order.order_status != OrderStatus.open or ix > 0:
                    ...
                    self.ws_client.cancel(order.order_id, order.instrument_name)
    ...
    def run_loop(self):
        ...
        while True:
            ...
            elif isinstance(parsed_message, PublicGetTickerResultSchema):
                self.on_ticker_update(parsed_message)
            else:
    ...
    def setup_session(self):
        ...
        markets = [f"{base}-{quote}" for base in BASES for quote in QUOTES]
        [self.ws_client.subscribe_ticker(market) for market in markets]
