...
class DeltaHedgerRfqQuoter:
    ...
    async def on_position_update(self, positions):
        # Here we could implement delta hedging logic based on position updates.
        pass

    async def on_balance_update(self, List[BalanceUpdateSchema]):
        # Here we could implement logic based on balance updates.
    ...
    async def run(self):
        ...
        await self.client.private_channels.balances_by_subaccount_id(
            subaccount_id=str(SUBACCOUNT_ID),
            callback=self.on_balance_update,
        )
...
async def main():
    ...
    rfq_quoter = DeltaHedgedRfqQuoter(client)
    while True:
        try:
