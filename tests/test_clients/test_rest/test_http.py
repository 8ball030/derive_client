...
def _create_unpriced_legs(client):
    # Derive RPC 10004: Multiple currencies not supported  [data={'subaccount_currency': 'ETH', 'base_asset_currency': 'BTC', 'note': 'sometimes due to risk caching of instruments, local tests will create new currency_id without risk updating cache'}]
...
def test_rfq_full_lifecycle(client_admin_wallet, client_owner_wallet):
    ...
    for unpriced_leg in rfq.legs:
        ticker = maker.markets.get_ticker(unpriced_leg.instrument_name)
